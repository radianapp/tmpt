...
let PDFDocument, PDFName, PDFHexString, PDFString, PDFDict, PDFArray, PDFRawStream, PDFNumber, PDFRef;

function initPDFLib() {
  if (PDFDocument) return;
  if (!window.PDFLib) {
    throw new Error('Pustaka pdf-lib belum dimuat di window.PDFLib.');
  }
  ({ PDFDocument, PDFName, PDFHexString, PDFString, PDFDict, PDFArray, PDFRawStream, PDFNumber, PDFRef } = window.PDFLib);
}
...
export async function decryptPDF(pdfBytes, password) {
  ...
  try {
    ...
    if (!encryptParams) {
      throw new Error('Dokumen PDF ini tidak terenkripsi.');
    }
    ...
    if (encryptParams.algorithm === 'AES-256') {
      ...
      if (!fileKey) {
        throw new Error('Kata kunci salah.');
      }
    ...
    // Hapus parameter /Encrypt dari trailer
    ...
    if (error.message.includes('tidak terenkripsi') ||
        error.message.includes('Kata kunci salah') ||
        error.message.includes('Unsupported encryption')) {
      throw error;
    }
    throw new Error(`Gagal membuka sandi PDF: ${error.message}`);
  }
}
...
export async function isEncrypted(pdfBytes) {
  ...
  try {
    ...
    throw new Error(`Gagal membaca berkas PDF: ${error.message}`);
  }
}
