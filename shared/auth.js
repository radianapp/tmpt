...
const AuthModule = {
     ...
     */
    async init() {
        console.log("[TMPT] Inisialisasi Auth...");
        ...
        if (storedKey && isUnlocked) {
            try {
                ...
                console.log("[TMPT] Auth Init Selesai. Unlocked: true");
                return true;
            } catch (e) {
        ...
        console.log("[TMPT] Auth Init Selesai. Unlocked: false");
        return false;
    },

    /**
     * Unlock the vault with a Kata Kunci Utama
     */
    async unlock(password) {
        ...
        if (!meta || typeof meta !== 'object' || !meta.salt_enc) {
            throw new Error("Data Tmpt rusak atau format tidak valid. Silakan gunakan fitur Pulihkan Data (Import) dari file backup.");
        }
        ...
        try {
            ...
            if (!testPayload) throw new Error("Tmpt kosong atau rusak.");
            ...
            if (decErr.name === 'InvalidCharacterError' || (decErr.message && decErr.message.includes('atob'))) {
                throw new Error("File Tmpt rusak atau telah dimodifikasi (Karakter Ilegal).");
            }
            
            // OperationError biasanya berarti password salah (MAC mismatch)
            if (decErr.name === 'OperationError') {
                throw new Error("Password Salah atau File Tmpt mungkin telah dimodifikasi.");
            }
